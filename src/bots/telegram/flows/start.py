...
from src.bots.telegram.util import log_enriched, valid_update
...
@log_enriched
async def start(
    update: Update, context: ContextTypes.DEFAULT_TYPE, log: "Logger" = loguru.logger
) -> None:
    if not valid_update(update):
        raise Exception("Invalid update", update.to_dict())
    session = get_session()

    ts_service = TelegramUserService(session, log)
    ...
    if potential_user is None:
        log.info("user not found, created new", new_user=user.model_dump())
        await update.message.reply_text(
            f"Hello, welcome new user @{user.telegram_username}!"
        )
    else:
        log.info("user found", existing_user=user.model_dump())
        await update.message.reply_text(
